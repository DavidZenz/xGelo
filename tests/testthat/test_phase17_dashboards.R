...
test_that("atomic batch|inventory|hash|size|rollback|idempotency", {
  ...
  expect_error(api$phase17_promote_batch(materialized$batch_root, public,
                                         injectors = list(read_back = function() stop("Injected read-back"))),
               "Injected read-back")
  expect_true(api$phase17_snapshot_equal(incumbent, api$phase17_snapshot_bytes(incumbent_path, root = public)))
  empty_public <- file.path(candidate_parent, "empty-public")
  dir.create(empty_public, recursive = TRUE)
  fresh_candidate <- file.path(candidate_parent, "fresh-candidate")
  fresh <- script$phase17_materialize_routes(bundles, fresh_candidate, batch_id)
  script$phase17_write_batch_envelope(fresh$batch_root, fresh$payloads, fresh$routes, batch_id)
  expect_silent(api$phase17_promote_batch(fresh$batch_root, empty_public))
  expect_silent(api$phase17_validate_batch_envelope(empty_public))
  sys.source(file.path(phase17_test_project_root, "R/dashboard/payload_nations_league.R"), api)
  vector_payload <- api$phase17_payload_nations_league(api$phase17_fixture_bundle("uefa_nations_league_2026_27"))
  vector_payload$edition_id <- api$phase17_editions()
  expect_error(api$phase17_validate_payload(vector_payload), "scalar")
