...
function renderEloEvolution(ratings, search){
  ...
  const lines = seriesItems.map(item => {
    ...
    return `<g class="elo-series"><path class="elo-step-line${item.selected ? " selected" : ""}${item.muted ? " muted" : ""}" d="${item.path}" stroke="${item.color}"><title>${esc(item.name)} ${Math.round(item.series.points[0].rating)} to ${Math.round(item.series.last.rating)}</title></path><path class="elo-hit-line" tabindex="0" role="button" aria-label="${esc(item.name)}" data-team="${item.teamAttr}" d="${item.path}"></path>${label}</g>`;
  }).join("");
