...
function renderEloEvolution(ratings, search){
  ...
  const lines = teamSeries.map((series, index) => {
    ...
    const labelClass = `elo-step-label${selected ? " selected" : ""}${hasSelection && !selected ? " dimmed" : ""}`;
    const label = showLabel ? `<text class="${labelClass}" tabindex="0" role="button" data-team="${teamAttr}" x="${(width - margin.right + 8).toFixed(1)}" y="${(labelByTeam[series.team] + 4).toFixed(1)}" fill="${color}">${esc(name)} ${deltaLabel}</text>` : "";
    return `<g class="elo-series"><path class="elo-step-line${selected ? " selected" : ""}${muted ? " muted" : ""}" d="${path}" stroke="${color}"><title>${esc(name)} ${Math.round(series.points[0].rating)} to ${Math.round(series.last.rating)}</title></path><path class="elo-hit-line" tabindex="0" role="button" aria-label="${esc(name)}" data-team="${teamAttr}" d="${path}"></path>${label}</g>`;
  }).join("");
  const note = search
  ...
  target.innerHTML = `<div class="elo-evolution-head"><div class="elo-evolution-title">Elo evolution</div><div class="elo-evolution-note">${esc(note)}${esc(selectionNote)} | ${esc(dateLabel(minT))}-${esc(dateLabel(maxT))}</div></div><svg class="elo-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="Elo evolution line-step plot"><line class="elo-axis" x1="${margin.left}" y1="${height - margin.bottom}" x2="${width - margin.right}" y2="${height - margin.bottom}"></line><line class="elo-axis" x1="${margin.left}" y1="${margin.top}" x2="${margin.left}" y2="${height - margin.bottom}"></line>${grid}${lines}</svg>`;
