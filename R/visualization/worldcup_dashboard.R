...
simulate_group_stage_dashboard <- function(
  ...
  display_position <- integer(nrow(counts))
  displayed_expected_points <- floor(expected_points * 10 + 0.5) / 10
  for (group_id in unique(counts$group)) {
    ...
    display_order <- idx[order(
      -displayed_expected_points[idx],
      -counts$group_win_count[idx] / n_tournaments,
      -counts$top_two_count[idx] / n_tournaments,
      -expected_points[idx],
      -expected_goal_difference[idx],
      -expected_goals_for[idx],
      counts$display_team[idx]
    )]
    display_position[display_order] <- seq_along(display_order)
  }
  ...
  expected_group_tables <- expected_group_tables[order(expected_group_tables$group, expected_group_tables$display_position), ]
...
<details id="releasePanel" class="release-details" aria-live="polite"><summary>Release details</summary><div class="release-details-body"></div></details>
</main>
...
const eloEvolutionSelectedTeams = new Set();
const toggleEloEvolutionTeam = team => {
  if (!team) return;
  if (eloEvolutionSelectedTeams.has(team)) eloEvolutionSelectedTeams.delete(team);
  else eloEvolutionSelectedTeams.add(team);
  renderEloRatings();
};
...
const teamMetaByTeam = {};
const teamMetaByDisplay = {};
function normalizeTeamKey(value){
  return String(value || "").trim().toLowerCase();
}
function mergeTeamMeta(existing, incoming){
  const merged = Object.assign({}, existing || {}, incoming || {});
  if ((!merged.fifa_code || !String(merged.fifa_code).trim()) && existing && existing.fifa_code) {
    merged.fifa_code = existing.fifa_code;
  }
  return merged;
}
function registerTeamMeta(row){
  if (!row) return;
  const team = row.team || "";
  const display = row.display_team || row.team_display || team;
  const fifa = row.fifa_code ? String(row.fifa_code).trim().toUpperCase() : "";
  if (!team && !display) return;
  const meta = { team, display_team: display, fifa_code: fifa };
  [team, display].forEach(value => {
    const key = normalizeTeamKey(value);
    if (!key) return;
    teamMetaByTeam[key] = mergeTeamMeta(teamMetaByTeam[key], meta);
    teamMetaByDisplay[key] = mergeTeamMeta(teamMetaByDisplay[key], meta);
  });
}
function registerMatchTeam(row, side){
  registerTeamMeta({
    team: row && row[`${side}_team`],
    display_team: row && row[`${side}_display`],
    fifa_code: row && row[`${side}_fifa_code`]
  });
}
(data.groups || []).forEach(registerTeamMeta);
(data.stage_probabilities || []).forEach(registerTeamMeta);
(data.group_probabilities || []).forEach(registerTeamMeta);
(data.current_group_tables || []).forEach(registerTeamMeta);
(data.expected_group_tables || []).forEach(registerTeamMeta);
(data.group_standings || []).forEach(registerTeamMeta);
(data.elo_ratings || data.elo_table || []).forEach(registerTeamMeta);
(data.team_form || data.team_forms || []).forEach(registerTeamMeta);
(data.matches || data.group_matches || []).forEach(row => {
  registerMatchTeam(row, "home");
  registerMatchTeam(row, "away");
});
(data.bracket_paths || []).forEach(row => {
  registerTeamMeta({ team: row.slot1_team, display_team: row.slot1_display });
  registerTeamMeta({ team: row.slot2_team, display_team: row.slot2_display });
  registerTeamMeta({ team: row.projected_winner_team, display_team: row.projected_winner });
  registerTeamMeta({ team: row.actual_winner_team, display_team: row.actual_winner });
});
function teamMetaFor(team, name){
  return teamMetaByTeam[normalizeTeamKey(team)] ||
    teamMetaByDisplay[normalizeTeamKey(team)] ||
    teamMetaByTeam[normalizeTeamKey(name)] ||
    teamMetaByDisplay[normalizeTeamKey(name)] ||
    {};
}
function teamFlag(team, name){
  const meta = teamMetaFor(team, name);
  const code = meta.fifa_code ? String(meta.fifa_code).trim().toUpperCase() : "";
  const flag = flagEmoji(code);
  if (flag) return `<span class="bracket-flag" aria-hidden="true">${esc(flag)}</span>`;
  return code ? `<span class="bracket-flag code" aria-hidden="true">${esc(code)}</span>` : "";
}
function teamLabel(team, name){
  const meta = teamMetaFor(team, name);
  const displayName = name || meta.display_team || team || "";
  return `<span class="bracket-team-label">${teamFlag(team, name)}<span>${esc(displayName)}</span></span>`;
}
function heatCell(value, className = ""){
  const prob = Number(value);
  const emptyClass = className ? ` ${className}` : "";
  if (!Number.isFinite(prob)) return `<td class="heat-cell${emptyClass} empty"></td>`;
  const rounded = Number((100 * prob).toFixed(1));
  if (rounded === 0) return `<td class="heat-cell${emptyClass} empty"></td>`;
...
function advancementStatus(row){
  const r32 = finiteProb(row.round_of_32_probability);
  if (r32 !== null && r32 >= 0.999) return {className: "qualified", symbol: "&#10003;", label: "Qualified for Round of 32"};
  if (r32 !== null && r32 <= 0.001) return {className: "eliminated", symbol: "&#215;", label: "Eliminated from Round of 32"};
  return null;
}
function statusMark(status){
  if (!status) return "";
  return `<span class="status-mark ${esc(status.className)}" title="${esc(status.label)}" aria-label="${esc(status.label)}">${status.symbol}</span>`;
}
function statusCell(status){
  return `<td class="status-cell">${statusMark(status)}</td>`;
}
function metricValueHtml(displayValue){
  return `<span class="metric-value"><span>${esc(displayValue)}</span></span>`;
}
function valueHeatCell(value, range, displayValue, className = ""){
  ...
  if (!Number.isFinite(numeric) || !Number.isFinite(minValue) || !Number.isFinite(maxValue)) {
    return `<td class="heat-cell${emptyClass} empty"><span class="heat-val">${metricValueHtml(displayValue)}</span></td>`;
  }
  const span = maxValue - minValue;
  const scaled = span > 0 ? (numeric - minValue) / span : (maxValue > 0 ? 1 : 0);
  const heat = scaled <= 0 ? 0 : Math.max(0.10, Math.min(0.92, 0.12 + scaled * 0.78));
  const strong = scaled >= 0.62 ? " strong" : "";
  const empty = heat === 0 ? " empty" : "";
  return `<td class="heat-cell ${className}${strong}${empty}" style="--heat:${heat.toFixed(3)};--prob:${Math.max(0, Math.min(1, scaled)).toFixed(3)}"><span class="heat-val">${metricValueHtml(displayValue)}</span></td>`;
}
...
function bracketForecastDetailHtml(g, projectedWinnerProbability){
  ...
  const slot1Label = teamLabel(g.slot1_team || "", slot1Name);
  const slot2Label = teamLabel(g.slot2_team || "", slot2Name);
...
function renderReleasePanel(){
  ...
  const body = panel.querySelector(".release-details-body");
  const release = data.metadata.release_metadata || {};
  if (!release.available) {
    panel.classList.add("alert");
    panel.setAttribute("role", "alert");
    summary.textContent = "No approved release available";
    body.innerHTML = `<div class="release-panel-title">Approved release unavailable</div><div class="release-panel-status">Forecast consumers must resolve a validated release manifest before model loading.</div>`;
    return;
  }
  ...
  const primaryView = release.primary_probability_view === "calibrated_1x2" ? "calibrated 1X2" : "raw 1X2";
  summary.textContent = `${statusText} · ${release.release_id}`;
  body.innerHTML = `<div class="release-panel-head"><div><div class="release-panel-title">${esc(statusText)}: ${esc(release.release_id)}</div><div class="release-panel-status">Model ${esc(release.selected_model_id)} | ${esc(release.track_id)} | G=${esc(release.score_support_g)} | Primary probabilities: ${esc(primaryView)}</div></div><a href="#release-provenance" id="openRelease">Open approved release</a></div><div class="release-panel-status">Scoreline distribution: fitted goal model</div><details class="release-provenance" id="release-provenance"><summary>Release provenance</summary><div class="release-provenance-grid"><strong>Root</strong><code>${esc(root)}</code><strong>Decision SHA-256</strong><code>${esc(release.decision_sha256)}</code><strong>Manifest SHA-256</strong><code>${esc(release.manifest_self_sha256)}</code><strong>Raw fallback</strong><span>${esc(release.raw_fallback_status || "n/a")}</span></div></details>`;
}
renderReleasePanel();
function renderHero(){
  ...
  const currentGroupRows = data.current_group_tables || [];
  const groupStageComplete = completedCount >= 72 && currentGroupRows.length > 0;
  ...
  const qualifiedFavoriteMetric = topQualifiedFavorite
    ...
    : unresolvedMetric;
  const bestGroupStageRow = [...currentGroupRows].sort((a,b)=>Number(b.points)-Number(a.points) || Number(b.goal_difference)-Number(a.goal_difference) || Number(b.goals_for)-Number(a.goals_for) || Number(b.wins)-Number(a.wins) || String(a.display_team).localeCompare(String(b.display_team)))[0];
  const completedGroupSummaries = Object.values(by(currentGroupRows,"group")).map(rows => {
    const sorted = [...rows].sort((a,b)=>Number(a.current_position)-Number(b.current_position));
    ...
    return {
      ...
      winner,
      runnerUp,
      pointsGap: Number(winner.points) - Number(runnerUp.points),
      goalDifferenceGap: Number(winner.goal_difference) - Number(runnerUp.goal_difference),
      goalsForGap: Number(winner.goals_for) - Number(runnerUp.goals_for)
    };
  }).filter(Boolean).sort((a,b)=>a.pointsGap-b.pointsGap || Math.abs(a.goalDifferenceGap)-Math.abs(b.goalDifferenceGap) || Math.abs(a.goalsForGap)-Math.abs(b.goalsForGap) || String(a.group).localeCompare(String(b.group)));
  const closestGroupFinish = completedGroupSummaries[0];
  const bestGroupStageMetric = bestGroupStageRow
    ? {label:"Best group-stage record", value:`${bestGroupStageRow.display_team} (${bestGroupStageRow.group})`, note:`${intFmt(bestGroupStageRow.points)} pts · ${Number(bestGroupStageRow.goal_difference) > 0 ? "+" : ""}${intFmt(bestGroupStageRow.goal_difference)} GD`}
    : qualifiedFavoriteMetric;
  const closestGroupFinishNote = closestGroupFinish
    ? (closestGroupFinish.pointsGap === 0
      ? (closestGroupFinish.goalDifferenceGap !== 0 ? "Decided on goal difference" : (closestGroupFinish.goalsForGap !== 0 ? "Decided on goals scored" : "Decided by tie-breaker"))
      : `${intFmt(closestGroupFinish.pointsGap)}-point gap`)
    : "Final standings";
  const closestGroupFinishMetric = closestGroupFinish
    ? {label:"Closest group finish", value:`Group ${closestGroupFinish.group}`, note:closestGroupFinishNote}
    : {label:"Closest group finish", value:"—", note:"Final standings"};
  const fourthMetric = groupStageComplete
    ? bestGroupStageMetric
  ...
  const fifthMetric = groupStageComplete
    ? closestGroupFinishMetric
...
function renderGroups(){
  ...
  document.getElementById("groupsGrid").innerHTML = Object.keys(probs).sort().map(g => {
    ...
    const statusByTeam = Object.fromEntries(rows.map(r => [r.team, advancementStatus(r)]));
    const standingRanges = {
      points: {min: 0, max: Math.max(0, ...currentRows.map(r => Number(r.points)))},
      goal_difference: {
        min: Math.min(0, ...currentRows.map(r => Number(r.goal_difference))),
        max: Math.max(0, ...currentRows.map(r => Number(r.goal_difference)))
      }
    };
    const forecastTable = `<div class="group-view" data-view="forecast"><table class="group-table"><thead><tr><th>Team</th><th class="status-head" aria-label="Status"></th><th class="num xpts-head">xPts<br>Avg</th><th class="num">1</th><th class="num">2</th><th class="num">3</th><th class="num">4</th><th class="num">R32</th><th class="num">3rd<br>Top 8</th></tr></thead><tbody>${rows.map(r => {
      const e = (exp[r.team] && exp[r.team][0]) || {};
      const code = r.fifa_code || "";
      return `<tr><td class="team-cell"><div class="team-ident"><span class="team-flag" aria-hidden="true">${esc(flagEmoji(code))}</span><span class="team-name">${esc(r.display_team)}</span></div></td>${statusCell(statusByTeam[r.team])}<td class="xpts-cell">${metricValueHtml(Number(e.expected_points).toFixed(1))}</td>${heatCell(r.position_1_probability)}${heatCell(r.position_2_probability)}${heatCell(r.position_3_probability)}${heatCell(r.position_4_probability)}${heatCell(r.round_of_32_probability, "qual")}${heatCell(r.third_place_qual_probability, "third")}</tr>`;
    }).join("")}</tbody></table></div>`;
    const currentTable = `<div class="group-view" data-view="current" hidden><table class="group-table current-table"><thead><tr><th>Team</th><th class="status-head" aria-label="Status"></th><th class="num xpts-head">Pts</th><th class="num standing-head">Played</th><th class="num standing-head">Wins</th><th class="num standing-head">Draws</th><th class="num standing-head">Losses</th><th class="num standing-head">Goals<br>For</th><th class="num standing-head">Goals<br>Against</th><th class="num standing-head">Goal<br>Diff</th></tr></thead><tbody>${currentRows.map(r => {
      const code = r.fifa_code || "";
      const gd = Number(r.goal_difference);
      const gdText = gd > 0 ? `+${gd}` : `${gd}`;
      return `<tr><td class="team-cell"><div class="team-ident"><span class="team-flag" aria-hidden="true">${esc(flagEmoji(code))}</span><span class="team-name">${esc(r.display_team)}</span></div></td>${statusCell(statusByTeam[r.team])}${valueHeatCell(r.points, standingRanges.points, intFmt(r.points), "standing")}<td class="standing-cell">${intFmt(r.played)}</td><td class="standing-cell">${intFmt(r.wins)}</td><td class="standing-cell">${intFmt(r.draws)}</td><td class="standing-cell">${intFmt(r.losses)}</td><td class="standing-cell">${intFmt(r.goals_for)}</td><td class="standing-cell">${intFmt(r.goals_against)}</td>${valueHeatCell(r.goal_difference, standingRanges.goal_difference, gdText, "standing")}</tr>`;
    }).join("")}</tbody></table></div>`;
...
function matchIsCompleted(row){
  return row.is_completed === true || row.is_completed === "TRUE" || row.match_status === "final";
}
function renderMatches(){
  ...
  const cardHtml = r => {
    ...
    const completed = matchIsCompleted(r);
  ...
  const upcomingRows = rows.filter(r => !matchIsCompleted(r));
  const completedRows = rows.filter(matchIsCompleted);
  const completedDivider = completedRows.length
    ? `<div class="match-divider">Decided matches</div>`
    : "";
  document.getElementById("matchesGrid").innerHTML = [
    ...upcomingRows.map(cardHtml),
    completedDivider,
    ...completedRows.map(cardHtml)
  ].join("");
}
...
function renderBracketInspector(g){
  ...
  panel.innerHTML = `<div class="bracket-inspector-head"><div><h2>Bracket match details</h2><div class="bracket-inspector-note">${esc(g.match_id)} | ${esc(g.round)}</div></div><span class="chip primary">${esc(g.projected_winner || "Projected winner")} ${pct(projectedWinnerProbability)}</span></div><div class="bracket-inspector-legend">${bracketHoverLegend()}</div>${detail}`;
...
function renderBracket(){
  ...
  const games = data.bracket_paths.map(g => {
    ...
    const slot1Name = g.slot1_display || g.slot1_label;
    const slot2Name = g.slot2_display || g.slot2_label;
    const slot1Target = slot1Team ? `<span class="bracket-team-target${slot1Winner ? " actual-winner" : ""}" tabindex="0" data-team="${esc(slot1Team)}" title="Hover to highlight ${esc(slot1Name)} path">${teamLabel(slot1Team, slot1Name)}</span>` : `<span>${esc(slot1Name)}</span>`;
    const slot2Target = slot2Team ? `<span class="bracket-team-target${slot2Winner ? " actual-winner" : ""}" tabindex="0" data-team="${esc(slot2Team)}" title="Hover to highlight ${esc(slot2Name)} path">${teamLabel(slot2Team, slot2Name)}</span>` : `<span>${esc(slot2Name)}</span>`;
...
function renderEloEvolution(ratings, search){
  ...
  const visibleTeamSet = new Set(teamSeries.map(series => series.team));
  const activeSelectedTeams = new Set([...eloEvolutionSelectedTeams].filter(team => visibleTeamSet.has(team)));
  const hasSelection = activeSelectedTeams.size > 0;
  const width = 1000;
  const height = 286;
  ...
  const labelCandidates = teamSeries.filter(series => activeSelectedTeams.has(series.team));
  const defaultLabelSeries = teamSeries
    .slice(0, search ? Math.min(12, teamSeries.length) : 8)
    .filter(series => !activeSelectedTeams.has(series.team));
  const labelBase = [...labelCandidates, ...defaultLabelSeries]
  ...
  const seriesItems = teamSeries.map((series, index) => {
    const color = palette[index % palette.length];
    const selected = activeSelectedTeams.has(series.team);
    const showLabel = labelByTeam[series.team] != null;
    const muted = hasSelection ? !selected : !showLabel;
    ...
    const path = stepPath(series.points);
    const teamAttr = esc(series.team);
    const labelY = labelByTeam[series.team];
    return {series, color, selected, showLabel, muted, name, deltaLabel, path, teamAttr, labelY};
  });
  const lines = seriesItems.map(item => {
    const labelClass = `elo-step-label${item.selected ? " selected" : ""}${hasSelection && !item.selected ? " dimmed" : ""}`;
    const label = item.showLabel ? `<text class="${labelClass}" tabindex="0" role="button" data-team="${item.teamAttr}" x="${(width - margin.right + 8).toFixed(1)}" y="${(item.labelY + 4).toFixed(1)}" fill="${item.color}">${esc(item.name)} ${item.deltaLabel}</text>` : "";
    const gainLabels = item.selected ? item.series.points.slice(1).map((point, index) => {
      const previous = item.series.points[index];
      const gain = Math.round(point.rating - previous.rating);
      if (gain === 0) return "";
      const resultingRating = Math.round(point.rating);
      const labelX = Math.max(margin.left + 18, Math.min(width - margin.right - 18, x(point.t)));
      const labelY = Math.max(margin.top + 10, Math.min(height - margin.bottom - 8, y(point.rating) - 8));
      return `<text class="elo-gain-label" x="${labelX.toFixed(1)}" y="${labelY.toFixed(1)}" text-anchor="middle" fill="${item.color}">${gain >= 0 ? "+" : ""}${gain} (${resultingRating})</text>`;
    }).join("") : "";
    return `<g class="elo-series"><path class="elo-step-line${item.selected ? " selected" : ""}${item.muted ? " muted" : ""}" d="${item.path}" stroke="${item.color}"><title>${esc(item.name)} ${Math.round(item.series.points[0].rating)} to ${Math.round(item.series.last.rating)}</title></path><path class="elo-hit-line" tabindex="0" role="button" aria-label="${esc(item.name)}" data-team="${item.teamAttr}" d="${item.path}"></path>${gainLabels}${label}</g>`;
  }).join("");
  const pickerRows = seriesItems.map(item => {
    const rowClass = `elo-picker-row${item.selected ? " selected" : ""}${hasSelection && !item.selected ? " dimmed" : ""}`;
    return `<div class="${rowClass}" tabindex="0" role="button" data-team="${item.teamAttr}"><span class="elo-picker-swatch" style="background:${item.color}"></span><span class="elo-picker-name">${esc(item.name)}</span><span class="elo-picker-delta">${esc(item.deltaLabel)}</span></div>`;
  }).join("");
  const note = search
    ? `${teamSeries.length} matching team${teamSeries.length === 1 ? "" : "s"}`
    : `${teamSeries.length} teams by current Elo`;
  const selectionNote = hasSelection ? ` | ${activeSelectedTeams.size} selected` : "";
  target.innerHTML = `<div class="elo-evolution-head"><div class="elo-evolution-title">Elo evolution</div><div class="elo-evolution-note">${esc(note)}${esc(selectionNote)} | ${esc(dateLabel(minT))}-${esc(dateLabel(maxT))}</div></div><div class="elo-evolution-body"><svg class="elo-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="Elo evolution line-step plot"><line class="elo-axis" x1="${margin.left}" y1="${height - margin.bottom}" x2="${width - margin.right}" y2="${height - margin.bottom}"></line><line class="elo-axis" x1="${margin.left}" y1="${margin.top}" x2="${margin.left}" y2="${height - margin.bottom}"></line>${grid}${lines}</svg><div class="elo-picker">${pickerRows}</div></div>`;
  target.querySelectorAll("[data-team]").forEach(element => {
    element.addEventListener("click", event => {
      event.preventDefault();
      toggleEloEvolutionTeam(element.dataset.team);
    });
...
function renderEloRatings(){
  ...
  const stageMax = Object.fromEntries(stageColumns.map(col => {
    const values = data.stage_probabilities.map(row => finiteProb(row[col])).filter(value => value !== null);
    return [col, Math.max(0, ...values)];
  }));
  const eloProbCell = (value, column) => {
    const prob = finiteProb(value);
    if (prob === null) return `<td class="elo-prob-cell empty"></td>`;
    const rounded = Number((100 * prob).toFixed(1));
    if (rounded === 0) return `<td class="elo-prob-cell empty">${pctNum(prob)}%</td>`;
    const scaled = stageMax[column] > 0 ? prob / stageMax[column] : 0;
    const heat = Math.max(0.05, Math.min(0.46, 0.06 + scaled * 0.36));
    const strong = scaled >= 0.62 ? " strong" : "";
    return `<td class="elo-prob-cell${strong}" style="--heat:${heat.toFixed(3)};--prob:${Math.max(0, Math.min(1, scaled)).toFixed(3)}">${pctNum(prob)}%</td>`;
  };
  const ratings = data.stage_probabilities
    .slice()
    .sort((a,b)=>Number(b.rating)-Number(a.rating))
    .map((row, index) => ({...row, elo_rank: index + 1}))
    .filter(row => {
      const group = String(row.group || "").toLowerCase();
      const groupLabel = `group ${group}`;
      return row.display_team.toLowerCase().includes(search) || group.includes(search) || groupLabel.includes(search);
    });
  const allRatings = data.stage_probabilities.map(row => Number(row.rating)).filter(Number.isFinite);
  ...
  document.getElementById("eloRows").innerHTML = ratings.map(row => {
    ...
    const ratingDisplay = Number.isFinite(rating) ? Math.round(rating) : "";
    const width = Number.isFinite(rating) ? Math.max(0.03, Math.min(1, (rating - minRating) / ratingSpan)) : 0;
