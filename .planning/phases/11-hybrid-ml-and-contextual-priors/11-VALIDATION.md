...
| 11-03-01 | 03 | 2 | HYBRID-02 | T11-10, T11-12, T11-13 | Context values are point-in-time, provenance-tracked, centroid-parented, common-panel eligible, and compatible with inherited benchmark contracts | unit + contract | `Rscript -e 'testthat::test_file("tests/testthat/test_hybrid_context_features.R", stop_on_failure = TRUE, stop_on_warning = TRUE)'`<br>`Rscript -e 'testthat::test_file("tests/testthat/test_benchmark_contracts.R", stop_on_failure = TRUE, stop_on_warning = TRUE)'` | no - depends on W0 | pending |
| 11-03-02 | 03 | 2 | HYBRID-02 | T11-11, T11-12 | Context bundle and individual ablations are registered, adapter-dispatched, runner-scored, and preserve the 630 open denominator | unit + registry + contract | `Rscript -e 'testthat::test_file("tests/testthat/test_hybrid_context_features.R", stop_on_failure = TRUE, stop_on_warning = TRUE)'`<br>`Rscript -e 'testthat::test_file("tests/testthat/test_benchmark_contracts.R", stop_on_failure = TRUE, stop_on_warning = TRUE)'` | no - depends on W0 | pending |
...
| 11-04-03 | 04 | 3 | HYBRID-04 | T11-16, T11-17 | Structural information affects only sparse evidence through continuous registered shrinkage, not raw RF predictors, and routes through common benchmark contracts when valid | unit + contract | `Rscript -e 'testthat::test_file("tests/testthat/test_hybrid_structural_prior.R", stop_on_failure = TRUE, stop_on_warning = TRUE)'`<br>`Rscript -e 'testthat::test_file("tests/testthat/test_benchmark_contracts.R", stop_on_failure = TRUE, stop_on_warning = TRUE)'` | no - depends on W0 | pending |
