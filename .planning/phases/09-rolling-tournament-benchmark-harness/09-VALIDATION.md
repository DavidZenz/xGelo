...
| Task ID | Plan | Wave | Requirement | Threat Ref | Secure Behavior | Test Type | Automated Command | File Exists | Status |
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 09-05-01 | 05 | 4 | BENCH-03 | T-09-G05-01, T-09-G05-02, T-09-G05-03 | The real hybrid-feature producer captures source presence/date before imputation, distinguishes observed zero from missing-then-zero, and adapters emit resolving runtime feature groups | producer + integration | `rtk Rscript --vanilla -e 'testthat::test_file("tests/testthat/test_transfermarkt_benchmark.R", reporter = "summary", stop_on_failure = TRUE, stop_on_warning = TRUE); testthat::test_file("tests/testthat/test_benchmark_baselines.R", reporter = "summary", stop_on_failure = TRUE, stop_on_warning = TRUE)'` | ✅ existing | ⬜ pending |
...
| 09-08-02 | 08 | 7 | BENCH-01, BENCH-02, BENCH-03, BENCH-04, BENCH-05 | Fail-fast acceptance independently checks output SHA/bytes, streaming rows, self-hash, the complete sorted parent graph, full tests, direct/transitive target isolation, and protected outputs; fresh goal verification is verifier-owned | independent acceptance + regression | `set -e; rtk git diff --quiet -- outputs/dashboard outputs/evaluation/wc2026; rtk Rscript --vanilla -e 'source("R/benchmark/registry.R"); source("R/evaluation/proper_scores.R"); source("R/benchmark/contracts.R"); source("R/benchmark/runner.R"); root <- "outputs/benchmarks/rolling_tournaments/phase09-baselines-frozen"; m <- read.csv(file.path(root, "manifests/checksum_manifest.csv"), check.names = FALSE); out <- m[m$artifact_role == "output", ]; count_rows <- function(path) { con <- file(path, "r"); on.exit(close(con)); n <- 0L; repeat { x <- readLines(con, 100000L, warn = FALSE); if (!length(x)) break; n <- n + length(x) }; n - 1L }; stopifnot(all(vapply(seq_len(nrow(out)), function(i) count_rows(file.path(root, out$relative_path[i])) == out$rows[i], logical(1)))); body <- benchmark_runner_sort(m[m$artifact != "checksum_manifest", ], "checksum_manifest"); self <- canonical_benchmark_sha256(body, c("artifact", "relative_path")); stopifnot(identical(tolower(m$canonical_content_sha256[m$artifact == "checksum_manifest"]), self)); input <- m[m$artifact_role == "input", ]; input <- input[order(input$artifact), ]; parent <- benchmark_runner_hash(paste(input$canonical_content_sha256, collapse = "|")); stopifnot(all(tolower(out$parent_hashes) == parent)); net <- targets::tar_network(targets_only = TRUE, outdated = FALSE); stopifnot("hybrid_goal_training_features_file->benchmark_phase09_predictions" %in% paste(net$edges$from, net$edges$to, sep = "->")); testthat::test_dir("tests/testthat", reporter = "summary", stop_on_failure = TRUE, stop_on_warning = TRUE)'; rtk git diff --quiet -- outputs/dashboard outputs/evaluation/wc2026` | ✅ existing | ⬜ pending |
