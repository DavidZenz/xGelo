...
1. **What are the exact undocumented response paths behind the Nations League `matches-calendar` and `standings` components?**
   - What we know: The official page bootstrap exposes service hostnames and the exact `competitionId`/season parameters needed to discover the backing structured payloads. `[CITED: https://www.uefa.com/uefanationsleague/fixtures-results/][CITED: https://www.uefa.com/uefanationsleague/standings/]`
   - What's unclear: The stable request path templates and full response schemas are not documented in the inspected official pages. `[CITED: https://www.uefa.com/uefanationsleague/fixtures-results/][CITED: https://www.uefa.com/uefanationsleague/standings/]`
   - Recommendation: Make the first implementation task a bounded discovery spike that captures one fixtures payload and one standings payload as raw-byte fixtures, then freeze schema assertions from those captured examples before writing general loaders. `[VERIFIED: 13-CONTEXT.md]`

2. **Should the Phase 13 “competition status” resource come from a backing service, an embedded match object, or an official pre-draw article JSON-LD extract?**
   - What we know: Match pages inline structured match and round status data, and EURO pre-draw articles expose official structured metadata with explicit dates and lifecycle facts. `[CITED: https://www.uefa.com/uefanationsleague/match/2048003--netherlands-vs-germany/][CITED: https://www.uefa.com/euro2028/news/029f-1f2ff991e87b-345fffcd69c3-1000--uefa-euro-2028-qualifying-draw-to-take-place-in-belfast/]`
   - What's unclear: Whether UEFA also exposes one edition-level status service response that is cleaner than deriving status from the discovered payload set. `[CITED: https://www.uefa.com/uefanationsleague/fixtures-results/]`
   - Recommendation: Plan the contract so `status` is its own required artifact type, but defer the final acquisition source choice until the first captured sample responses are inspected. `[VERIFIED: 13-CONTEXT.md]`
