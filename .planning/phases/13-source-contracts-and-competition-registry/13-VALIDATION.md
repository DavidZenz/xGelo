...
- [ ] `tests/testthat/test_phase13_source_contracts.R` - Plan 13-04 acquisition regression for normalized accepted fixtures, preserved UEFA source values, and the normalized EURO pre-draw empty schema.
...
|----------|-------------|------------|-------------------|
| Confirm the live official UEFA structured service paths and response shapes remain available at capture time. | DATA-01 | UEFA service paths are public but undocumented and may change independently of the parser contract. | After the automated Wave 5 gate, run the bounded capture/discovery command with one operator-supplied HTTPS structured URL for each of fixtures, groups, standings, results, and status; inspect the accepted manifest and normalized fixtures, and retain resulting raw bytes locally. A failure must leave the prior accepted bundle active. |
