...
| **Baseline capture** | Wave 0 runs the full suite in a child process, records exit status plus combined-output SHA-256 in `16-BASELINE.md`, and fingerprints the STATE.md failure `156 fixture IDs paired with zero-length normalized source columns`. |
| **Feedback latency target** | under 30 seconds for fast smoke; under 60 seconds for focused Phase 16 tests |

## Acceptance Policy

The full suite is a regression comparison, not a claim of green status while the known baseline persists. Phase 16 acceptance requires the focused Phase 16 suite and relevant Phase 13-15 regressions to pass, and requires the final full-suite run to have **no failures beyond the Wave 0 baseline fingerprint**. A persistent known baseline failure is reported separately with its exit status, output hash, and signature from `16-BASELINE.md`; it is never silently swallowed or relabelled as a passing full suite.
...
| Wave 1 | 16-01 | 2 | 16-01-01, 16-01-02 | Activation, active-after-draw, pre_draw, and D-04 branches pass |
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 16-00-01 | 00 | 0 | COMP-03, COMP-04, SIM-02, SIM-04 | T-16-00-01 | Fast smoke and child-process full-suite baseline are fingerprinted; known failure is separate. | harness/baseline | `Rscript --vanilla -e 'testthat::test_file("tests/testthat/test_phase16_euro_qualifying.R", filter="phase16_smoke", stop_on_failure=TRUE, reporter="summary")'` | no - Wave 0 | pending |
...
| 16-03-01 | 03 | 3 | SIM-02 | T-16-11, T-16-13, T-16-14 | Calibrated seeded simulation requires exact Phase 15 interim-stage handoff, confirmed kickoff, and active resolved inputs. | unit/integration/replay | `Rscript --vanilla -e 'testthat::test_file("tests/testthat/test_phase16_euro_qualifying.R", filter="simulation|handoff|ranking_stage|rng|suppression", stop_on_failure=TRUE, reporter="summary")'` | no - Wave 0 | pending |
...
| 16-05-02 | 05 | 5 | COMP-03, SIM-02, SIM-04 | T-16-22, T-16-23, T-16-24, T-16-25 | Atomic revision retention, both D-04 branches, fresh-process identical hashes, current pre_draw output, and no-new-failure baseline policy are verified. | integration/replay/publication | `Rscript --vanilla -e 'testthat::test_file("tests/testthat/test_phase16_euro_qualifying.R", filter="publication|revision|continuity|replay|fresh_process|pre_draw", stop_on_failure=TRUE, reporter="summary")' && Rscript --vanilla scripts/build_euro_qualifying_outcomes.R --replay-check` | no - Wave 0 | pending |
...
- [ ] A child-process full-suite baseline capture records exit status, combined output, SHA-256 fingerprint, date, and the known STATE.md signature `156 fixture IDs paired with zero-length normalized source columns` in `16-BASELINE.md`.
...
- [x] Wave 0 explicitly creates the focused test file, baseline fingerprint, and all shared fixtures required by later tasks.
- [x] No watch-mode flags are used.
- [x] Fast smoke feedback is targeted below 30 seconds; focused feedback is targeted below 60 seconds.
- [x] `nyquist_compliant: true` is justified by the explicit Wave 0 task, task map, focused commands, regression commands, replay commands, and final baseline comparison policy above.
