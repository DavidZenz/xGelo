...
| **Fast smoke command** | `Rscript --vanilla -e 'testthat::test_file("tests/testthat/test_phase16_euro_qualifying.R", desc="phase16_smoke", stop_on_failure=TRUE, reporter="summary")'` |
...
| **Baseline capture** | Wave 0 runs the exact-path helper after the smoke attempt without a short-circuit: capture mode writes `16-BASELINE.md` even when the child full suite exits nonzero, records that child exit status, sorted normalized failing identities, complete-output SHA-256, and known-baseline disposition, and returns zero only after its record is persisted. |
| **Feedback latency target** | under 30 seconds for fast smoke; under 60 seconds for focused Phase 16 tests |

## Acceptance Policy

The full suite is a regression comparison, not a claim of green status while the known baseline persists. Wave 0 runs `set +e; Rscript --vanilla -e 'testthat::test_file("tests/testthat/test_phase16_euro_qualifying.R", desc="phase16_smoke", stop_on_failure=TRUE, reporter="summary")'; smoke_status=$?; Rscript --vanilla .planning/phases/16-euro-qualifying-activation-and-play-off-rules/16-baseline-check.R --capture --baseline .planning/phases/16-euro-qualifying-activation-and-play-off-rules/16-BASELINE.md; capture_status=$?; if [ "$smoke_status" -ne 0 ] || [ "$capture_status" -ne 0 ]; then exit 1; fi` so a smoke failure cannot suppress baseline creation. Capture mode returns zero after writing the record even when the child full suite is nonzero; it returns nonzero only for a helper/recording failure. Phase 16 acceptance requires the focused Phase 16 suite and relevant Phase 13-15 regressions to pass, then requires `.planning/phases/16-euro-qualifying-activation-and-play-off-rules/16-baseline-check.R --compare --baseline .planning/phases/16-euro-qualifying-activation-and-play-off-rules/16-BASELINE.md` to exit successfully. The comparator permits zero current failures or exactly the recorded pre-existing failure identity/signature, treats the output hash as reported evidence, rejects any new or unparseable failure, and reports a persistent known baseline separately with exit status, normalized identities, output hash, and disposition; it is never silently swallowed or relabelled as a passing full suite.
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 16-00-01 | 00 | 0 | COMP-03, COMP-04, SIM-02, SIM-04 | T-16-00-01 | Fast smoke and exact-path child-process full-suite capture record exit status, normalized identities, output hash, and known-baseline disposition; capture persists a nonzero child result and known failure is separate. | harness/baseline | `set +e; Rscript --vanilla -e 'testthat::test_file("tests/testthat/test_phase16_euro_qualifying.R", desc="phase16_smoke", stop_on_failure=TRUE, reporter="summary")'; smoke_status=$?; Rscript --vanilla .planning/phases/16-euro-qualifying-activation-and-play-off-rules/16-baseline-check.R --capture --baseline .planning/phases/16-euro-qualifying-activation-and-play-off-rules/16-BASELINE.md; capture_status=$?; if [ "$smoke_status" -ne 0 ] || [ "$capture_status" -ne 0 ]; then exit 1; fi` | yes - Wave 0 | complete |
| 16-00-02 | 00 | 0 | COMP-03, COMP-04, SIM-02, SIM-04 | T-16-00-02 | Active-after-draw, pre_draw, four-host, topology, and interim-stage fixtures are deterministic and local. | fixture/contract | `Rscript --vanilla -e 'testthat::test_file("tests/testthat/test_phase16_euro_qualifying.R", desc="fixture", stop_on_failure=TRUE, reporter="summary")'` | yes - Wave 0 | complete |
...
| 16-01-03 | 01 | 1 | COMP-03, SIM-04 | T-16-26 | The committed EURO row remains `pre_draw` until complete acceptance; both normal accepted-publication branches persist `scheduled` only after acceptance, failure injection creates no scheduled-without-accepted row, the loader admits a complete scheduled bundle, and the next state build reloads that lifecycle and lineage. | integration/lifecycle | `Rscript --vanilla -e 'testthat::test_file("tests/testthat/test_phase16_euro_qualifying.R", filter="lifecycle_activation|registry_path|date_only|scheduled|pre_draw_guard|normal_fallback|normal_normalized|failure_injection|no_scheduled_without_accepted", stop_on_failure=TRUE, reporter="summary")'` | no - Wave 0 | pending |
...
| 16-05-01 | 05 | 5 | COMP-03, SIM-02, SIM-04 | T-16-21, T-16-22 | Registered CLI builds exact inventory from config/manifest inputs, validates the explicit EURO edition flag, and validates exact D-16 payload fields. | integration/CLI | `Rscript --vanilla -e 'testthat::test_file("tests/testthat/test_phase16_euro_qualifying.R", filter="cli|inventory|dry_run|payload_copy|active_after_draw", stop_on_failure=TRUE, reporter="summary")' && Rscript --vanilla scripts/build_euro_qualifying_outcomes.R --edition-id uefa_euro_2028_qualifying --dry-run` | no - Wave 0 | pending |
| 16-05-02 | 05 | 5 | COMP-03, SIM-02, SIM-04 | T-16-22, T-16-23, T-16-24, T-16-25 | Atomic revision retention, both D-04 branches, fresh-process identical hashes, current pre_draw output, and the runnable no-new-failure comparator are verified; the comparator is not gated by a preceding short-circuit. | integration/replay/publication | `set +e; Rscript --vanilla -e 'testthat::test_file("tests/testthat/test_phase16_euro_qualifying.R", filter="publication|revision|continuity|replay|fresh_process|pre_draw", stop_on_failure=TRUE, reporter="summary")'; focused_status=$?; Rscript --vanilla scripts/build_euro_qualifying_outcomes.R --edition-id uefa_euro_2028_qualifying --replay-check; replay_status=$?; Rscript --vanilla .planning/phases/16-euro-qualifying-activation-and-play-off-rules/16-baseline-check.R --compare --baseline .planning/phases/16-euro-qualifying-activation-and-play-off-rules/16-BASELINE.md; baseline_status=$?; if [ "$focused_status" -ne 0 ] || [ "$replay_status" -ne 0 ] || [ "$baseline_status" -ne 0 ]; then exit 1; fi` | no - Wave 0 | pending |

## Wave 0 Requirements

- [x] `tests/testthat/test_phase16_euro_qualifying.R` exists with a repository-root-aware loader, `phase16_smoke`, fixture constructors, and focused contract sections.
- [x] Wave 0 runs the exact-path capture helper after the smoke attempt without a short-circuit; the child-process full-suite baseline records its nonzero exit status when present, normalized failing identities, complete-output SHA-256 fingerprint, date, and the known STATE.md signature `156 fixture IDs paired with zero-length normalized source columns` in `16-BASELINE.md`. Capture mode returns zero after the record is written and nonzero only for a helper/recording failure.
- [x] The exact comparison command `Rscript --vanilla .planning/phases/16-euro-qualifying-activation-and-play-off-rules/16-baseline-check.R --compare --baseline .planning/phases/16-euro-qualifying-activation-and-play-off-rules/16-BASELINE.md` returns zero for zero failures or the recorded pre-existing identity/signature, exits nonzero only for a new or unparseable failure, and reports a persistent baseline as non-green; output-hash changes are reported evidence, not a standalone failure.
- [x] The active-after-draw fixture has real groups, stable teams/fixtures, confirmed kickoffs, zero completed results, and zero completed standings; it is asserted as distinct from pre_draw.
- [x] Four covered hosts, zero/one/two consumed host branches, all three topologies, and preserved scenario rows are available as deterministic local fixtures.
- [x] Phase 15 handoff fixtures include valid `team_id` rows with `ranking_scope = interim_overall` and `ranking_stage = interim_overall`, plus final-only, wrong-stage, duplicate, missing, and unresolved variants.
- [x] Exact D-16 heading/body/date/refresh/source/reason fields are fixture assertions, not manual-only checks.
...
| Lifecycle persistence | Both normal `phase13_acquire_publish_refresh()` branches persist `scheduled` only through `phase13_transition_competition_edition()` after a complete accepted bundle; failure injection cannot create scheduled-without-accepted state, loader validation admits only a complete scheduled bundle, and the next state path observes the persisted row. |
